...
@section('content-header')
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Data Progam</h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    {{ Breadcrumbs::render('manageProgram') }}
                </ol>
            </div>
            <!-- /.col -->
        </div>
            ...
            <div class="card-header">
                ...
                <div class="modal fade" id="modal-AddData">
                    <div class="modal-dialog">
                        <form action="{{ route('manageProgram.store') }}" method="post">
                            @csrf
                ...
                <tbody>
                    @forelse ($program as $index => $item)
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="card">
                                            <div class="card-header bg-danger">
                                                <h1>{{ $item->kode }}</h1>
                                                <h4><sub>{{ $item->nama_program }}</sub></h4> <br>
                                    ...
                                    <div class="modal-footer justify-content-between">
                                        <button type="button" class="btn btn-default" data-dismiss="modal">
                                            Close</button>
                                        <form action="{{ route('manageProgram.destroy', $item->id) }}" method="post">
                                            @csrf
                                            @method('delete')
                        ...
                        <div class="modal fade" id="modal-Edit{{ $item->id }}">
                            <div class="modal-dialog">
                                <form action="{{ route('manageProgram.update', $item->id) }}" method="post">
                                    @csrf
                                    @method('put')
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h4 class="modal-title">Edit Data {{ $item->kode }} </h4>
                                        ...
                                        <div class="modal-body">
                                            <label for="">Kode</label>
                                            <input type="text" name="kode" id="" class="form-control"
                                                value="{{ old('kode') ?? $item->kode }}" placeholder="Kode Program">
