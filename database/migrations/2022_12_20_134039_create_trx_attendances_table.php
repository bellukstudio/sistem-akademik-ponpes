...
class CreateTrxAttendancesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('trx_attendances', function (Blueprint $table) {
            ...
            $table->foreignId('presence_type')->references('id')
                ->on('master_attendances')->onDelete('no action')->onUpdate('cascade');
            $table->string('category_attendance')->nullable();
            $table->string('other_category', 20)->nullable();
